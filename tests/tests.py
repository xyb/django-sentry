...
class SentryTest(BaseTestCase):
    ...
    def test_vars_should_be_string(self):
        try:
            i = 1
            f = 1.2
            b = True
        ...
        last = Message.objects.all().order_by('-id')[0:1].get()
        vars = dict(last.data['__sentry__']['frames'][-1]['vars'])
        self.assertEqual(vars['i'], '1')
        self.assertEqual(vars['f'], '1.2')
        self.assertEqual(vars['b'], 'True')
        self.assertEqual(vars['s'], "'string'")
        self.assertEqual(vars['u'], "u'unicode'")
        self.assertEqual(vars['t'], "('a', 0)")
        self.assertEqual(vars['l'], "['b', 2]")
        self.assertEqual(vars['d'], "{'y': 2, 'x': 1}")
        self.assertTrue(vars['o'].startswith('<object object at '))
        self.assertTrue(vars['o'].endswith('>'))
        self.assertEqual(vars['exc'],
                "ZeroDivisionError('integer division or modulo by zero',)")
        self.assertEqual(vars['self'],
                '<tests.tests.SentryTest testMethod=test_vars_should_be_string>')
